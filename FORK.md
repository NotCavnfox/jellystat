...
This is a homelab fork of `https://github.com/CyferShepard/Jellystat.git`, maintained to add features and accept
contributions. Image published to `ghcr.io/notcavnfox/jellystat`.
...
- **Tests:** `test/smoke.sh` (boot + HTTP 3000/) and `test/compose.test.yml`
  must stay green — that gate protects the live homelab.

## Local loop
```bash
docker build -t localbuild:ci .
PORT=3000 HEALTH_PATH=/ IMAGE=localbuild:ci bash test/smoke.sh
